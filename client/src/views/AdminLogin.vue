    ...
    async loginButton() {
      ...
      if (!this.isEmpty) {
        try {
          ...
          const response = await ApiV1.post('/v1/api/admin/login', userCredentials)

          const authToken = response.headers['x-auth-token']
          if (authToken) {
            localStorage.setItem('x-auth-token', authToken)
            this.$router.push({ name: 'home', params: { isAdmin: true } }).then(() => {
              location.reload() // Ensure the closing parentheses are correct
            })
          } else {
          ...
          if (error.response && error.response.status === 404) {
            this.usernameError = 'there in no admin account with this username'
